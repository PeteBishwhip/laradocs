...
| `slug` | string | Override the URL slug — see [Routing](/docs/navigation/routing). |
...
| `group` | string | Bucket the page under a sidebar heading and a top-level tab — see [Grouping](/docs/navigation/grouping). |
...
| `image` | string | Social / OG image URL — see [SEO](/docs/seo). |
| `redirect` | string | 301-redirect this slug to another page or absolute URL. |
| `priority` | float | Override the [sitemap](/docs/seo/sitemap) `<priority>` for this page (`0.0`–`1.0`). |
...
> block. See the [SEO guide](/docs/seo) for the full reference.
...
- `group:` produces both a sidebar section heading and a top-level tab — see [Grouping](/docs/navigation/grouping).
...
`tags` also drive the auto-generated [tag index pages](/docs/navigation/tags):
...
[SEO guide](/docs/seo) for site-wide defaults and examples.
