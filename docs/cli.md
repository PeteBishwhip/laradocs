...
- **Unresolved icons** — an `icon:` front-matter value or an inline `@icon()` call that does
  ...
  [Icons](/docs/content/icons#linting-icon-references).
...
[Customising stubs](/docs/customisation/stubs).
...
navigation tree, generates the [sitemap](/docs/seo/sitemap), and
...
Builds the full-text [search](/docs/navigation/search) index and pushes it to
