...
final class VersionsCommand extends Command
{
    ...
    public function handle(VersionRegistry $versions): int
    {
        ...
        if ($all === []) {
            $this->info('No documentation versions detected. Enable versioning and add version directories or `versions.available` entries.');
        ...
        $this->table(
            ...
            array_map(
                function (VersionInfo $info) use ($latest): array {
                    return [
                        $info->key,
                        $info->label,
                        $info->semver ?? '',
